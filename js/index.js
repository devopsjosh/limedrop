...
<div>
	...
	//****************************
	/* This is for importing an external file with a list of items*/
	//****************************
	...
	//****************************
	/* This loads a saved list stored in localstorage */
	//****************************
	$("#load-btn").click(function () {
		...
		Object.keys(itemLists).map(key => (
			$("#item-list-select").append("<option value='" + key + "'>" + key + "</option>")
		))
	...
	function writeToFile(file, data) {
		var password = String($("#ld-login-pw").val());
		API.emit("put", "\\d2bs\\kolbot\\data\\web\\", file, data, password, function (err, result) {
			if (err) {
				console.log(err);
			}
			console.log(result);
	...
	//****************************
	/* This exports a list to a downloadable file */
	//****************************
	...
	//****************************
	/* This saves a list to localstorage */
	//****************************
	$("#save-btn").click(function () {
		var itemList = getDropQueueList();
		var itemLists = JSON.parse(window.localStorage.getItem("itemLists")) || {};
		var listName = prompt("Please enter the list name");
		itemLists[listName] = itemList
		window.localStorage.setItem("itemLists", JSON.stringify(itemLists))
	});
